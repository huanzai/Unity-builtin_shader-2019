...
{
    // unity_ColorSpaceDielectricSpec = vec4(0.04, 0.04, 0.04, 1-0.04);
    // 金属表面会吸收所有的折射光线，所以:
    // 1. metallic 为 1 时，纯金属，没有漫反射（折射光）
    // 2. metallic 为 0 时，绝缘体，大部分都发生了折射（1-0.04）
    // 就是说：oneMinusReflectivity 的意思是“光发生折射的比例”，折射都作用在漫反射（diffuseColor)
    // 但注意：oneMinusReflectivity 考虑了金属度，当金属度为1时，它的值为 1，当金属度为0时，它的值为（1-0.04）

    // specColor 表示 F0 也就是垂直向下（cos(0)）看这个材料时，它的反射率
...
{
    // tangentSign 由DCC软件中的切线自动生成,和顶点的环绕顺序有关。
    // 是模型的Scale值是三维向量，即xyz，当这三个值中有奇数个值为负时（1个或者3个值全为负时），unity_WorldTransformParams.w = -1，否则为1.
...
half3 ShadeSHPerVertex (half3 normal, half3 ambient)
{
    ...
    // L2 部分在顶点进行，L0..L1 在 pixel 中进行，注意计算时需要判断 gamma 空间
    #else

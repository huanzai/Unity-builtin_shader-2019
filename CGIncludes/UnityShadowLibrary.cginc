...
fixed UnitySampleBakedOcclusion (float2 lightmapUV, float3 worldPos)
{
        ...
        // unity_ShadowMask 贴图有 4 个通道，所以只能记录 4 个灯光的光照，超过 4 个的灯，都默认使用 baked
        // rawOcclusionMask 记录着 4 个灯光在当前点的光照情况
        // forward rendering path 每个 pass 只能处理一个光，所以要从采样结果 rawOcclusionMask 中进行筛选
        // unity_OcclusionMaskSelector 记录的就是当前 pass 所处理的灯光编号，第 0 个 (1,0,0,0)，第 1 个(0,1,0,0) ...
        // dot(rawOcclusionMask, unity_OcclusionMaskSelector) 就拿到了当前光的信息
        // 参考:https://blog.csdn.net/wodownload2/article/details/105178552

        // 从 shadowmask 贴图中读取光照情况（应该是深度值），暂不考虑 UNITY_LIGHT_PROBE_PROXY_VOLUME 的情况
...
{
    // unity_ShadowFadeCenterAndType.w : 存的是 Shadow Projection 的值，1 表示 Stable Fit，0 表示 Close Fit
    // unity_ShadowFadeCenterAndType.xyz = camPos + normalize(camFront) * 0.26666f * shadowDistance
    // 其中 shadowDistance 为 Quality - Shadows - Shadow Distance 的值
    // sphereDist 为当前点到圆心点的距离
    float sphereDist = distance(wpos, unity_ShadowFadeCenterAndType.xyz);

    // 如果是 Stable Fit 返回 sphereDist，否则返回 z (相机的深度)
...
{
    // (_LightShadowData).x = 1 - shadowStrength;
    // (_LightShadowData).y = camera.GetFar() / shadowDistance;
    // const float shadowStartFade = shadowRadius - shadowDistance * kShadowFadeRange;
    // const float shadowFadeInvLen = 1.0f / (shadowRadius - shadowStartFade);
    // (_LightShadowData).z = shadowFadeInvLen;
    // (_LightShadowData).w = -shadowStartFade * shadowFadeInvLen;
    // 其中，kShadowFadeRange = 0.2
    // 总结：
    //      (圆心)-----startFade(0)----1/3----2/3----3/3
    // 就是说靠圆心到 startFade 之间 F 值为 0，然后 F 的值开始变大，一直到 >= ShadowRadius ，F 变成 1。
    // 参考：https://github.com/myl2232/PBR_GI_Light/wiki/Shadow

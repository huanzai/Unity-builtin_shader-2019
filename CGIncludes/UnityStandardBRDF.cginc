...
{
    // 粗糙度: roughness = 1 - smoothness
    // 半角向量: normalize(lightDir+viewDir)
...
#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0
    ...
    // Diffuse term 漫反射, 根据粗糙度算出(光的散射度*视线的散射度)
    // diffuseTerm = lightScatter * viewScatter * nl （最后的 nl 是 BRDF 公式中的一项，只不过这里展开了）
    ...
    // G 几何遮挡 这里将 BRDF 高光项的分子结合到了 G 中，所以变成了 V
    ...
    // 这里 * PI 是因为 diffuseTerm 理论上应该除以 PI，但是由于一些原因没有这么做，但是为了保持平衡，specularTerm * PI
...
#   endif
    ...
    specularTerm = max(0, specularTerm * nl); // 这里的 nl 也是 BRDF 公式中的一项，只不过展开了而已（想想 diffuseTerm 也乘 nl 了）
...
#   endif
    ...
    // 为何菲涅尔系数 F0 就是 specColor = lerp(vec3(0.04), albedo, metallic)
    // 最后一项如何理解: 间接光的高光项
    // 为何缺少了间接光高光部分的 BRDF Lut ？感觉最后一项的效果应该就是
